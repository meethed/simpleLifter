...
function addlifter(l) { //add lifter l to server
  ...
  .then(res => {console.log("lifter added");  loadfromserver();});

} //end function addlifter

function addFromBtn() {
  let s= compStatus.session;
  let newlot=Math.max(...globalLifters.map(e=>e.lot))+1;
  addlifter({"session":s,"lot":newlot,"name":"AAA","gp":"Z","year":"1990-12-31"});
}
