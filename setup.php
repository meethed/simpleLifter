...
<div class="setupbox">
  ...
  foreach ($result as $r) { 
    ...
    echo "<div class='tr'>";
    echo "<div class='td session-name' contenteditable id='s".$r["session"]."'>".$_SESSION["compName"]." - " .$d."</div>";
  ...
  //parse here but don't actually add it to the competition
   ...
   importFile.files[0].text().then(e => {
    ...
    csv.forEach(l => { //remove the header row
      ...
      newLifterData[i].gp=l[fl];
      newLifterData[i].gender=l[ge];
      newLifterData[i].gear=l[eq];
      newLifterData[i].lifts=l[li];
