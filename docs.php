...
<a target="_blank" href="./resources/tutorials/comprego.png"><img class="img-tut" src="./resources/tutorials/comprego.png"></a>
<ol>
<li>Click on <a href="./addcomp.php"> New Competition </a>on the menu</li>
<li>Input the competition name, choose your federation (ruleset), and start date</li>
<li>Choose the number of days, sessions and platforms (from 1-10) that the comp will run</li>
<li>Delete unnecessary sessions by highlighting them and clicking on them (eg if you only need 1 platform on the final day delete the other platforms for that day by clicking on them)</li>
<li>Choose an access code (note this is for all access - officials, referees, tech desk). There's no minimum and it's unencrypted. I generally pick something easy in case a referee accidentally logs out in the middle of a session! It'll help them get back online quickly.</li>
<li>Please consider providing an email so I can reach out. This is a mandatory field but if it's not a valid email I won't care. I'll probably reach out and say hi :)</li>
<li>The feature selection ensures you're not wasting your time with the wrong features. Some people want to use just the spreadsheet and have manual lights and separate timers. Others may only want to use the lights and timers, and pair simpleLifter with openLifter or LiftingCast. These are all options, depending on your use case.</li>
<li>Click submit and you're done!!</li>
</ol>
</div>
<div class="collapsible">Setup a small comp in simpleLifter</div>
<div class="collapsiblecontent">
<a target="_blank" href="./resources/tutorials/login.png"><img class="img-tut" src="./resources/tutorials/login.png"></a>
<ul>
...
<li>Add the lifter entry and weigh in data. Enter the division in the format M-CL-PL (1 letter for gender, 2 letters for equipment type, and 2 letters for the lifts). For the weight class, you must have the division set up first. When you put in weigh in data, the Class cell will be highlighted orange. Right click on it to set the lifter's weight class to align with their weigh in weight.</li>
<li>In the top left, set the active lift to Squat 1 and set the active Group to the starting group, and then you're good to go!</li>
</ul>
<br><br><br><br><a target="_blank" href="./resources/tutorials/compmanager.png"><img class="img-tut" src="./resources/tutorials/compmanager.png"></a>
...
<p>You'll often see sessions described as a 3 digit number. Each digit represents the day, session and platform respectively - ie. 111 is Day 1 Session 1 Platform 1.
<p>To assign lifters to a session, simply click in the day/session/platform (DSP) boxes on the right to assign a lifter to a particular session. Should you need to make last minute changes - say because a lifter has pulled out and flights or sessions must change - this is now very easy and there's no risk of losing data.</p>
<h4>Lot numbers</h4><p>Lot numbers can be generated randomly either for the entire competition (recommended) or by flights (eg. Flight A gets a lower lot number than Flight B). Note that once you press the number it'll immediately overwrite and upload the new numbers so please be careful!</p>
</div>
...
<div class="collapsiblecontent">
<p>Short version, add http://cotf.zapto.org/multiStream.php?c=XXX as a browser source to OBS.You can use the platform/session filter to only display the stream data from certain sessions...see below.</p>
<p>You must ensure that the Browser Course resolution is set to 1920x1080 (you'll know if you've got it wrong). It also helps to set a custom frame rate of 60fps as the animations will be a lot smoother.</p>
</div>

<div class="collapsible">Setup OBS scoreboard</div>
<div class="collapsiblecontent">
<p>Short version, add http://cotf.zapto.org/scoreboard.php?c=XXX as a browser source to OBS.</p>
<p>To customise the look and feel of it, you can modify the CSS code in OBS. I'll put up a guide on that eventually.</p>
</div>
