...
var timerto,
 ...
 isAttempting,
 isCentre=0,
 compName="<?php echo $_POST["compName"]; ?>".toUpperCase().split(",");
  if (!Array.isArray(compName)) compName=compName[0];
 var pos="<?php echo $_POST["pos"]; ?>", 
 loadedTimeout=0,
 whiteTimeout=0,
 redTimeout=0,
 blueTimeout=0,
 ...
 currenttimer,
 clockOffset=0,
 lastUpdate="2020-01-01 00:00:01",
 twoOffset,
 att,
 oldplt,
...
function checkUpdate() {
  compName.forEach(e => {
  fetch("lights.php?compLetters=" + e) //this is what happens every second - the stuff previous waits until it's ready (complete)
  .then((response) => response.json())
  .then((lightvars) => {
  		if (lightvars.updated>lastUpdate) { //if something has changed
